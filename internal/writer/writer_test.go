...
func makeTracesRecord(nRows int) arrowlib.Record {
	...
	for i := 0; i < nRows; i++ {
		...
		col++
		// Span fields
		b.Field(col).(*array.FixedSizeBinaryBuilder).Append(make([]byte, 16)) // trace_id
		col++
		b.Field(col).(*array.FixedSizeBinaryBuilder).Append(make([]byte, 8)) // span_id
		col++
		b.Field(col).(*array.StringBuilder).AppendNull() // trace_state
		col++
		b.Field(col).(*array.FixedSizeBinaryBuilder).AppendNull() // parent_span_id
		col++
